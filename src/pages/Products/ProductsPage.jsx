...
const ProductsPage = () => {
  ...
  // prevent stale responses from overwriting latest state
  ...
  // One slot reserved for AddProductCard
  ...
  const loadProducts = useCallback(
    async (page = 1) => {
      // create a new fetch id and mark as latest
      ...
      try {
        ...
        // only set error state if still latest
        if (fetchId === latestFetchIdRef.current) {
        ...
        // only turn off loading if this was the latest fetch
        if (fetchId === latestFetchIdRef.current) {
          setLoading(false);
        }
      }
  ...
  }, [currentPage, searchTerm, selectedCategory, itemsPerPage, loadProducts]);

  // Scroll to top after new products finish loading
  useEffect(() => {
