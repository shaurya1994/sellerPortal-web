...
const OrdersGrid = ({
  ...
  latestFetchIdRef,
  onMetaUpdate,
  onResetPage, // ✅ new prop to reset pagination
}) => {

  // ✅ default to "in-process" instead of "all"
  ...
  // ✅ when filters clear, reset tab back to "in-process"
  useEffect(() => {
    ...
    async (page = 1) => {
      ...
      try {
        const res = await fetchSellerOrders({
          ...
          // ✅ removed `all`; always send a real status
          status: orderType,
            ...
            {["in-process", "delivered"].map((type) => (
              <label
                ...
                onClick={() => {
                  if (orderType !== type) {
                    setOrderType(type);
                    onResetPage(); // ✅ tells parent to go to page 1
                  }
                }}
              >
                <input
